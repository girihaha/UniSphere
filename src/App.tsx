...
type Tab = 'clubs' | 'feed' | 'network' | 'profile' | 'moderation' | 'myposts';
...
function MainApp() {
  ...
  const renderPage = () => {
    switch (activeTab) {
      ...
      case 'feed':        return <FeedPage onOpenNotifications={() => setNotificationsOpen(true)} />;
      case 'network':     return <NetworkPage onOpenNotifications={() => setNotificationsOpen(true)} />;
      case 'moderation':  return <ModerationDashboard />;
      case 'myposts':     return <MyPostsPage />;
      case 'profile':
        return (
          <ProfilePage
            onLogout={logout}
  ...
  const navTab: 'clubs' | 'feed' | 'network' | 'profile' =
    activeTab === 'moderation' || activeTab === 'myposts' ? 'profile' : (activeTab as any);
